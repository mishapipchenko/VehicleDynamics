...
model Kinematics2
  ...
  parameter SIunits.Position upper_fore_i[3] = {86.868, 215.9, 200}/1000 annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position upper_aft_i[3] = {-95.25, 215.9, 200}/1000 annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position upper_o[3] = {-6.347, 523.24, 287.02}/1000 annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position tie_i[3] = {41.128, 215.9, 117.856}/1000 annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position tie_o[3] = {56, 532.333, 164.821}/1000 annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position lower_fore_i[3] = {83.376, 215.9, 90}/1000 annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position lower_aft_i[3] = {-95.25, 215.9, 90}/1000 annotation(
    Dialog(group = "Geometry"));
  parameter SIunits.Position lower_o[3] = {0, 556.499, 124.998}/1000 annotation(
    Dialog(group = "Geometry"));
...
// wheel vectors from the contact patch
 ...
 Modelica.Blocks.Sources.Ramp ramp(height = 0.0254, duration = 1, startTime = 0.1)  annotation(
    Placement(transformation(origin = {-57, -65}, extent = {{-7, -7}, {7, 7}})));
 Modelica.Blocks.Sources.Ramp ramp1(height = 2*0.0254, duration = 1, startTime = 0.1)  annotation(
    Placement(transformation(origin = {-127, -39}, extent = {{-5, -5}, {5, 5}})));
equation
...
// calculating the camber and toe
 ...
 connect(ramp.y, position.s_ref) annotation(
    Line(points = {{-50, -64}, {-22, -64}, {-22, -60}}, color = {0, 0, 127}));
 connect(ramp1.y, position1.s_ref) annotation(
    Line(points = {{-122, -38}, {-104, -38}, {-104, -42}}, color = {0, 0, 127}));
  annotation(
